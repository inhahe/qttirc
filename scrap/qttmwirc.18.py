#qttmw stands for Qt Twisted Matrix Windows
#todo:
...
import os, sys, time, re, itertools, traceback, copy
  ...
  if params[0].lower() == "/join":
    ...
    if "m" in switches:
      ...
      if len(params) == 1:
        ...
        network_config, bynetwork = lookupnetworkconfig(params[1])
    mainwin.tab_widget.setTabText(window.network.serverwindow.tab_index, network_config.name if network_config else params[1])
    ...
    network.serverwindow = serverwindow if "m" in switches else window.network.serverwindow
    ...
    scripts.update(loadscripts([network])) #should make a function that works on a single network and maybe a function that calls that function on a list of networks
  else:
...
class Network:
  def __init__(self, server, network_config, mynick=None):
    self.conn = None
    self.server = server
    self.isupport = {}
...
config = json_to_namespaces(json.load(open("qttmwirc.conf.json")))
...
if __name__=="__main__":
  ...
  for network_config in config.networks:
    ...
    if password:
      password = password.encode("ascii")
    realname = network_config.realname
    if realname:
      realname = realname.encode("ascii")
    network = Network(None, network_config)
    server = ServerFactory(nickname=nick, username=username, password=password, realname=realname, network=network)
    ...
    network.serverindex = 0
    networks.append(network)
    network.conn = reactor.connectTCP(network_config.servers[0][0].encode("ascii"), network_config.servers[0][1], server) 
