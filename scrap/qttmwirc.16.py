...
class ServerWindow(QWidget):
  def __init__(self, network):
    QWidget.__init__(self)
    self.network = network
    mainwin.tab_widget.addTab(self, network.config["name"])
    self.textwindow = QTextEdit(self)
    self.textwindow.setReadOnly(True)
    self.textwindow.setFont(font)
    self.textwindow.setStyleSheet("* {position: absolute; bottom: 0}") #doesn't work
...
class MainWindow(QMainWindow):
  ...
  def __init__(self):
    ...
    self.mainmenu.setCornerWidget(self.test, Qt.TopRightCorner) #debug
    ...
    self.newclientaction = QAction("&Server window")
    self.newclientaction.triggered.connect()
