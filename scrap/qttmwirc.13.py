...
#trivia plugin - react to things typed in from local, and show plugin's messages in window
...
class ServerConnection(irc.IRCClient):
  ...
  def privmsg(self, fromhostmask, msg):
    ...
    colorify(messagewindow, "<%s> %s" % (nick, msg))
    messagewindow.addline("")
...
class Channel:
  ...
  #  self.channelwindow.nickslist.setHtml("<br>".join(sorted(self.users.keys()))) #fortunately nicks can't have < or > in them. if i weren't lazy i would replace them anyway with &lt or whatever (assuming pyqt even supports that)
    self.window = ChannelWindow(self, network)
  ...
  def post(self, message):
    ...
    self.window.addline("<%s> %s" % (self.network.mynick, message))
  def rejoined(self):
    pass #todo
...
if __name__=="__main__":
  ...
  for network_config in config["networks"]:
    ...
    server = ServerFactory(nickname=nick, username=username, password=password, realname=realname)
    network = Network(server, network_config)
