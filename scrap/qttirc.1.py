#qttirc stands for Qt Twisted on Windows IRC
...
#when loading files like qttirc.conf.json, load from the path of the loaded python file
...
def lookupnetworkconfig(name): #name = network or server name
  ...
  for network_conf in config.networks:
    print(f'network_conf.name.encode("ascii").lower().decode() {network_conf.name.encode("ascii").lower().decode()}')
    if network_conf.name.encode("ascii").lower().decode() == name:
      return network_conf, True
      print("here 11")
    for server, port in network_conf.servers:
      print(f'server.encode("ascii").lower(): {server.encode("ascii").lower()}')
      if server.encode("ascii").lower().decode() == name:
...
def docommand(windowtype, window, text): #todo: test from server, channel, and privmsg
  ...
  if params[0].lower() == "/join":
    ...
    network = Network()
    print(f"network_config.nicks: {network_config.nicks}")
