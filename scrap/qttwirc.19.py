...
#when loading files like qttmwirc.conf.json, load from the path of the loaded python file
  ...
  def keyPressEvent(self, event):
    if event.key() == Qt.Key_Return:
      ...
      if text.lstrip().startswith("/"):
        ...
        self.serverwindow.addline("* Nothing performed. This is not a channel or private-message window")
...
def docommand(windowtype, window, text): #todo: test from server, channel, and privmsg
  ...
  if params[0].lower() == "/join":
    ...
    network_config = None
    switches = params.pop(1)[1] if params[1].startswith("-") else ""
    if len(params) > 3:
    ...
    if "m" in switches:
      ...
      if len(params) == 1:
        ...
        network_config, bynetwork = lookupnetworkconfig(params[1])
      if len(params) == 1:
      ...
      if len(params) == 1:
        ...
        network_config, bynetwork = lookupnetworkconfig(params[1])
    temp_win = window.network.serverwindow if window.network else window
    ...
    network = Network()
    nick = network_config.nicks[0] if network_config.nicks else config.nicks[0]
    realname = network_config.realname if network_config.realname else config.realname
    server = ServerFactory(nick.encode("ascii"), username=network_config.username.encode("ascii") if network_config.username else None,
                           password=network_config.password.encode("ascii") if network_config.password else None, realname=realname.encode("ascii") if realname else None, network=network)
    window.network = network
    ...
    window.network.conn = reactor.connectTCP(servername, port, server) #todo: change name of tab to current server
...
class ServerConnection(irc.IRCClient):
  ...
  def signedOn(self):
    ...
    self.signedon = True
    tab_widget.setTabText(tab_widget.indexOf(self.server.network.serverwindow), "%s - %s" % (self.server.network.config["name"], self.nickname)) #might be better to store the index
  ...
  def userRenamed(self, oldnick, newnick):
    ...
    if oldnick_lower in self.server.network.messagewindows:
      ...
      tab_widget.setTabText(tab_widget.indexOf(self), newnick) #might be better to store the index
  ...
  def irc_RPL_ISUPPORT(self, prefix, params):
    for param in params[1:]:
      ...
      if keyvalue[0] == "NETWORK":
        self.server.network.name=keyvalue[1]
        mainwin.tab_widget.setTabText(self.server.network.serverwindow.tab_index, keyvalue[1])
...
class ServerWindow(QWidget):
  ...
  def addlinecolored(self, *args): #todo: remove the redundancy between addline for server windows and addline for channel windows
    ...
    for arg in args:
      ...
      flag = not flag
    self.textwindow.insertHtml("<br>")
    self.textwindow.moveCursor(QTextCursor.End) #not sure this is necessary, it was in an example for doing this for some reason. oh.. i guess it's not since i think we have cursor disabled?
    scrollbar = self.textwindow.verticalScrollBar()
    scrollbar.setValue(scrollbar.maximum())
...
class Network(object):
  def __init__(self, server=None, network_config=None, mynick=None):
    self.conn = None
    self.server = server
    self.isupport = {}
...
if __name__=="__main__":
  ...
  config = json_to_namespaces(json.load(open(os.path.join(qttwircpath, "qttmwirc.conf.json")))) #todo: allow to specify config and performs in command-line options
