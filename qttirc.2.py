...
class ChannelWindowText(QTextEdit):
  ...
  def contextMenuEvent(self, e):
    ...
    if nick.startswith("nick:"):
      ...
      QTextEdit.contextMenuEvent(self, e)
...
class MainWindow(QMainWindow):
  ...
  def closeEvent(self, event):
    for network in networks:
      if network.server and network.server.serverconnection:
...
class ServerConnection(irc.IRCClient):
  ...
  def irc_RPL_ISUPPORT(self, prefix, params):
    ...
    for param in params[1:]:
      ...
      if keyvalue[0] == "NETWORK":
        ...
        mainwin.tab_widget.setTabText(self.server.network.serverwindow.tab_index, networkname + " - " + self.nickname)
  ...
  def irc_RPL_NAMREPLY(self, prefix, params):
    ...
    for nick in params[3].split():
      ...
      np = nick[:nmo.end(1)]
      nwp = nick[nmo.end(1):]
      nwp_lower = irc.irclower(nwp)
      user = self.server.network.users.get(nwp_lower, None)
      if user is None:
        user = User(nwp, self.server.network)
        self.server.network.users[nwp_lower] = user
      user.channels.add(params[2])
  ...
  def irc_RPL_SASLSUCCESS(self, prefix, params):
    addline(self.server.network.serverwindow, "* SASL authentication successful")
    
  def irc_ERR_SASLFAIL(self, prefix, params):
    addline(self.server.network.serverwindow, "* SASL authentication failed")
            
  def irc_ERR_SASLTOOLONG(self, prefix, params):
    addline(self.server.network.serverwindow, "* SASL authentication failed: too long")
  
  def irc_ERR_SASLABORTED(self, prefix, params):
    addline(self.server.network.serverwindow, "* SASL authentication failed: aborted")
  
  def irc_ERR_SASLALREADY(self, prefix, params):
    addline(self.server.network.serverwindow, "* SASL authentication failed: already authenticated")
  ...
  def kickedFrom(self, channelname, kicker, message):
    ...
    nick, ident, host = splithostmask(kicker)
    ...
    for x in range(nickslist.count()):
      nickslist.takeItem(0)
    channel.users = {} 
  ...
  def userKicked(self, kickednick, channelname, kicker, message):
    ...
    kickernick_lower = irc.irclower(kickernick)
   ...
   #   if kickerhost: user.host = kickerhost
    kickeruser = self.server.network.users[kickernick_lower]
    kickeruser.ident = kickerident
    kickeruser.host = kickerhost
    ...
    nickslist = channel.channelwindow.nickslist
    for x in range(nickslist.count()): #stack overflow says count()-1 for some reason
      item = nickslist.item(x)
      if item.user.nick == kickednick:
        nickslist.takeItem(x)
        break
    del channel.users[kickednick_lower]
    self.server.network.users[kickednick_lower].channels.remove(channelname)
  ...
  def userRenamed(self, oldnick, newnick):
    ...
    user = self.server.network.users[oldnick_lower]
    self.server.network.users[newnick_lower] = user
    del self.server.network.users[oldnick_lower]
  ...
  def userJoined(self, hostmask, channelname): #relies on a change to irc.py
    ...
    user = self.server.network.users.get(nick_lower, None)
    if user is None:
      user = User(nick, self.server.network)
      self.server.network.users[nick_lower] = user
    user.ident = ident
    user.host = host
  ...
  def userLeft(self, hostmask, channelname):
    ...
    nickslist = channel.channelwindow.nickslist
    for x in range(channel.channelwindow.nickslist.count()): #stack overflow says count()-1 for some reason
      item = channel.channelwindow.nickslist.item(x)
      if item.user.nick == nick:
        nickslist.takeItem(x)
        break
    del channel.users[nick_lower]
    self.server.network.users[nick_lower].channels.remove(channelname) #could there be a case mismatch here?
  ...
  def userQuit(self, hostmask, quitmsg):
    ...
    del self.server.network.users[nick_lower] #todo: do this with a delay so the user can do a local whowas kinda thing on the user yet we don't store all users forever (memory leak)
  ...
  def joined(self, channelname):
    ...
    app.processEvents()
    sizes = channel.channelwindow.splitter.sizes()
  ...
  def pong(self, user, secs):
    cw = mainwin.tab_widget.currentWidget()
    if cw.network == self.server.network:
      addline(cw, f"[{user} ping reply]: {secs:.3f}secs")
    else:
...
class ClientFactory(protocol.ClientFactory):
  ...
  def startedConnecting(self, connector):
    ...
    addline(self.network.serverwindow, "* Connecting to (%s, %s)" % tuple(self.network_config.servers[self.network.serverindex])) #todo: can addr and port be gotten from connector?
    pass
  ...
  def clientConnectionFailed(self, connector, reason):
    ...
    if self.network_config:
      ...
      addline(self.network.serverwindow, "* Connection to (%s, %d) failed" % (oldaddr, oldport)) #todo: add reason for fail
...
class identd(protocol.Protocol):
  def dataReceived(self, data):
    self.transport.write(data.strip() + " : USERID : UNIX : " + config.ident + "\r\n" )
...
#  mainwin.showMaximized()
  ...
  ding = QSound(os.path.join(qttircpath, r"sounds\01_ECHOBEL3.wav")) 
  ...
  if len(sys.argv)<=1:
    ...
    config = yaml_to_namespaces(yaml.load(open(sys.argv[1]), Loader=yaml.FullLoader))
